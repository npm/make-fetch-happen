...
class CacheEntry {
  ...
  async store (status) {
    // console.trace('called store', status)
    ...
    if (this.response.status === 200) {
      ...
      const cacheWritePromise = new Promise((resolve, reject) => {
        ...
        cacheWriteResolve = () => {
          // console.trace('cacheWriteResolve called')
          return resolve()
        }
      ...
      let abortStream
      // console.error(status, shouldBuffer)
      if (shouldBuffer) {
        // console.error('setting up collector')
        ...
        // console.error('somehow ended up in here')
        // TODO if the cache write fails, log a warning but return the response anyway
