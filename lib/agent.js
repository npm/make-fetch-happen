...
function getProxy (proxyUrl, opts, isHttps) {
  ...
  if (proxyUrl.protocol === 'http:' || proxyUrl.protocol === 'https:') {
    // which agent we use is based on the url we're making a request to, not the proxy
    if (isHttps) {
  ...
  if (proxyUrl.protocol.startsWith('socks')) {
    // our current socks proxy agent does not support an overridden dns lookup method, so will not
...
class UnsupportedProxyError extends Error {
  constructor (proxyUrl) {
    super(`unsupported proxy protocol: '${proxyUrl.protocol}'`)
    this.code = 'EUNSUPPORTEDPROXY'
    this.url = proxyUrl.toString()
  }
}
